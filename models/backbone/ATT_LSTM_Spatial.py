"""
attention LSTM + spatial affinity to predict pedestrian trajectory
"""
...
class SpatialAffinity(nn.Module):
    def __init__(self, args):
        super(SpatialAffinity, self).__init__()
        self.args = args
        # # https://github.com/huang-xx/TrafficPredict/blob/master/srnn/model.py
        self.embedding_size = 64
        self.embedding_linear = nn.Linear(self.args.input_size, self.embedding_size)
        self.att_linear= nn.Linear(self.args.hidden_size+self.embedding_size, 1)

        hidden1_size = 32
        hidden2_size = 64
        self.fc1 = torch.nn.Linear(args.input_size, hidden1_size)
        self.fc2 = torch.nn.Linear(hidden1_size, hidden2_size)
        self.fc3 = torch.nn.Linear(hidden2_size, args.hidden_size*self.args.bidirectional)

    def forward(self, input_traces, decoder_outputs):
        encoder_locations = self.cal_locations(input_traces)
        att_alpha_list = []
        for p in range(self.args.pedestrian_num):
            encoder_rs = self.cal_r(input_traces[:, p, :], input_traces)
            encoder_u = torch.cat((encoder_locations, encoder_rs), -1)
            att_alpha = self.att_linear(encoder_u).squeeze(-1)
            att_alpha_list.append(att_alpha)

        att_alpha_list = torch.stack(att_alpha_list, 1)
        outputs = torch.bmm(att_alpha_list, decoder_outputs)
        return outputs

    def cal_locations(self, input_traces):
        # # CIDNN's DecoderNet
        encoder_locations = []
        for i in range(self.args.pedestrian_num):
            input_trace = input_traces[:, i, :]
            encoder_location = F.relu(self.fc1(input_trace))
            encoder_location = F.relu(self.fc2(encoder_location))
            encoder_location = self.fc3(encoder_location)

            encoder_locations.append(encoder_location)

        # stack all person
        encoder_locations = torch.stack(encoder_locations, 1)
        return encoder_locations

    def cal_r(self, target_trace, input_traces):
        encoder_rs = []
        for j in range(self.args.pedestrian_num):
            relative_location = target_trace - input_traces[:, j, :]
            encoder_r = F.relu(self.embedding_linear(relative_location))
            encoder_rs.append(encoder_r)

        # encoder_rs size: (B, pedestrian_num, embedding_size)
        encoder_rs = torch.stack(encoder_rs, 1)
        return encoder_rs
